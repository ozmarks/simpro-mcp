// One deliberate, local version bump. Bumps package.json, syncs mcpb/manifest.json
// and version.json to match, commits all three, and creates the matching `v<version>` tag.
//
//   npm run release <patch|minor|major|x.y.z>
//
// The release pipeline (release-mcpb.yml) is driven by the tag and re-verifies the
// three files agree, so re-running a failed CI build never changes the version.
